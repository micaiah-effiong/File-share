...
fs.watch(path.resolve(__dirname, "files"), async (eventType: string, filename: string | Buffer) => {
	// if (eventType) return;
	...
	let isChanged = dirChange.map((e) => e.filename).join() !== appFiles.map((e) => e.filename).join();
	let newFile = dirChange.filter((file) => !(file.filename in appFilesObject))[0];
